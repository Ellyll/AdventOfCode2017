...
type PacketState = { Position : int*int ; Direction : Direction ; StepsTaken : int }
...
let tryMove packetState (diagram : DiagramSymbol[,]) =
    ...
    | HorizontalLine ->
        ...
        if isInBounds diagram (newR,newC) then
            ...
            | VerticalLine when (match currentSymbol with | VerticalLine | Cross | Letter _ -> true | _ -> false) ->
                Some ({ packetState with Position = (newR, newC) ; StepsTaken = packetState.StepsTaken + 1 })
            | HorizontalLine when (match currentSymbol with | HorizontalLine | Cross | Letter _ -> true | _ -> false) ->
                Some ({ packetState with Position = (newR, newC) ; StepsTaken = packetState.StepsTaken + 1 })
            | Cross
            | Letter _ -> Some ({ packetState with Position = (newR, newC) ; StepsTaken = packetState.StepsTaken + 1 })
            | HorizontalLine when currentSymbol = VerticalLine ->
                ...
                if isInBounds diagram (newR2,newC2) then
                    Some ({ packetState with Position = (newR2, newC2) ; StepsTaken = packetState.StepsTaken + 2 })
                else
                    None
    ...
    | Cross ->
        ...
        |> List.tryHead
        |> Option.map (fun ((newR, newC), newDirection, _) ->
            { packetState with Position = (newR,newC) ; Direction = newDirection ; StepsTaken = packetState.StepsTaken + 1 })
    | _ ->
...
let initialState = { Position = startPosition ; Direction = Down ; StepsTaken = 1 }

let finalState, _ = travel diagram (initialState, [])

printfn "Final state: %A" finalState
printfn "Answer: %i" finalState.StepsTaken
